...
t1 = np.linspace((0*np.pi)/3, (6*np.pi)/3, 100)
...
class Controller(Node):
    def __init__(self):
        ...
        self.pen1_subcriber = self.create_subscription(Pose2D, "/pen33_pose", self.bot1_callback, 100)
...
def main(args=None):
    ...
    while (1):
        #########################################################################################################################
        global bot1_index, bot1_prev_index, bot1_index, stop1_flag, stop2_flag, stop3_flag
        ...
        new_vel_x = (rot_pts(vel_x, vel_y, pose1_theta))[0]
        new_vel_y = (rot_pts(vel_x, vel_y, pose1_theta))[1]
      ...
      ############################################################################################################################
         ...
         #########################################################################################################################
        # global bot2_index, bot2_prev_index, bot2_index, stop1_flag, stop2_flag, stop3_flag
        # goal2_x = (x2[bot2_index])
        # goal2_y = (y2[bot2_index])
        # goal2_theta = 0
        # pose2_theta = con.pose2_theta
        # if pose2_theta > 180:
        #     pose2_theta = pose2_theta - 360
        # vel_x = goal_x - con.pose2_x
        # vel_y = goal_y - con.pose2_y
        # err_theta = (goal_theta - pose1_theta)
        # # print(err_theta)
